 ...
 * 输入构造类
 */
class AIKITAPI AIKIT_DataBuilder {
public:
    static AIKIT_DataBuilder* create();
    virtual ~AIKIT_DataBuilder();

    //设置输入数据控制参数对
    virtual AIKIT_DataBuilder* control(const char* key, AIKIT_ParamBuilder* builder) = 0;

    virtual AIKIT_DataBuilder* text(const char* key, const char* data, uint32_t dataLen, uint32_t dataStatus) = 0;
    virtual AIKIT_DataBuilder* textPath(const char* key, const char* path)               = 0;
    virtual AIKIT_DataBuilder* textFile(const char* key, const FILE* file)               = 0;

    virtual AIKIT_DataBuilder* audio(const char* key, const char* data, uint32_t dataLen, uint32_t dataStatus) = 0;
    virtual AIKIT_DataBuilder* audioPath(const char* key, const char* path)               = 0;
    virtual AIKIT_DataBuilder* audioFile(const char* key, const FILE* file)               = 0;

    virtual AIKIT_DataBuilder* image(const char* key, const char* data, uint32_t dataLen, uint32_t dataStatus) = 0;
    virtual AIKIT_DataBuilder* imagePath(const char* key, const char* path)               = 0;
    virtual AIKIT_DataBuilder* imageFile(const char* key, const FILE* file)               = 0;

    virtual AIKIT_DataBuilder* video(const char* key, const char* data, uint32_t dataLen, uint32_t dataStatus) = 0;
    virtual AIKIT_DataBuilder* videoPath(const char* key, const char* path)               = 0;
    virtual AIKIT_DataBuilder* videoFile(const char* key, const FILE* file)               = 0;
