...
AWAKENUP::AWAKENUP()
{
  awakeUpPub = awakenUpNH.advertise<std_msgs::Bool>("/WakenUp",50);
  awakeUpAudio = awakenUpNH.advertise<std_msgs::String>("/TextToVoice",100);
  ros::param::get("/awaken2Node/MSPLoginParam",MSPLoginParam = "appid = 6597fc2f,work_dir = .");
  ros::param::get("/awaken2Node/engine_start",engine_start = "ivw");
  ros::param::get("/awaken2Node/ivw_res_path",ivw_res_path = "fo|/home/goudan/srbot_ws-C/src/sr_voice/bin/msc/res/ivw/wakeupresource.jet");
  ros::param::get("/awaken2Node/ivw_threshold",ivw_threshold = "0:1450;1:1450;2:1450");
  ros::param::get("/awaken2Node/sst",sst = "wakeup");
...
int main(int argc, char *argv[])
{
  ...
  while (ros::ok())
  {
    ...
    if(AwakenUpTrueOrFalse)
    {
      AwakenUp.WakeUpResult(true,"主人我在。");
      AwakenUpTrueOrFalse = false;
      //AwakenUp.WakeUpResult(false,"nullptr",0);
    }
    ros::spinOnce();
    usleep(200*1000);
  }
